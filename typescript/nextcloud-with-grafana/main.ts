...
const CONFIG_PATH = join(YAMLS_PATH, 'config.yaml');
...
function victoriaMetrics(
  ...
  config.addFile(join(YAMLS_PATH, config_path), prom_config_name);
...
export class NextCloud extends Chart {
  constructor(scope: Construct, id: string, props: ChartProps = {}) {
    ...
    const DATABASE_CREDENTIALS_PATH = join(
      YAMLS_PATH,
      config.database_credentials_path,
    );
